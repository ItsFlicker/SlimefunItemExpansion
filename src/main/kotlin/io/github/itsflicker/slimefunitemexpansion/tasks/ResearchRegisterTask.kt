...
import io.github.itsflicker.slimefunitemexpansion.items.resources.*
...
object ResearchRegisterTask {
    fun run() {
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "GEARS_R"),
            100861, "齿轮", 15
        ).addItems(
            IEItems.WOODEN_GEAR,
        ...
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "OSMIUM_R"),
            100862, "锇", 12
        ).addItems(
        ...
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "MACHINE_BLOCK_R"),
            100863, "机械外壳", 10
        ).addItems(IEItems.MACHINE_BLOCK).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "ALLOY_FURNACE_R"),
            100864, "合金炉", 18
        ).addItems(IEItems.ALLOY_FURNACE).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "BASE_COIL_R"),
            100865, "基础线圈", 8
        ).addItems(IEItems.BASIC_COIL).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "ADVANCED_COIL_R"),
            100866, "高级线圈", 9
        ).addItems(IEItems.ADVANCED_COIL).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "LIGHTNING_WHIP_R"),
            100867, "浑元形意太极拳法", 30
        ).addItems(IEItems.LIGHTNING_WHIP, IEItems.MOUSE_TAIL_JUICE).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "DRILL_R"),
            100868, "手钻", 18
        ).addItems(
        ...
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "FARMING_R"),
            100869, "高级农业技术", 32
        ).addItems(IEItems.JIN_KE_LA, IEItems.GROW_CORE).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "COBBLESTONE_GENERATOR_R"),
            1008610, "造石机", 25
        ).addItems(IEItems.COBBLESTONE_GENERATOR, IEItems.COBBLESTONE_GENERATOR_2).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "ADVANCED_EXP_R"),
            1008611, "新型经验技术", 30
        ).addItems(
        ...
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "SPEED_TORCH_R"),
            1008612, "加速火把", 15
        ).addItems(IEItems.SPEED_TORCH).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "COMPRESSED_FREEZER"),
            1008613, "一键冷却剂工厂", 31
        ).addItems(IEItems.COMPRESSED_FREEZER).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "ENHANCED_GLOWSTONE_R"),
            1008614, "《强化》", 5
        )
            .addItems(IEItems.ENHANCED_GLOWSTONE, IEItems.ENHANCED_OBSIDIAN).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "OSGLOGLAS_R"),
            1008615, "锇荧黑曜合金", 12
        ).addItems(IEItems.OSGLOGLAS_INGOT).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "NEW_TECH_R"),
            1008616, "新科技,来临力", 19
        ).addItems(
        ...
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "NEW_GENERATOR_R"),
            1008617, "恒河里的发电机", 20
        ).addItems(
        ...
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "LAVA_FACTORY_R"),
            1008618, "岩浆工厂", 33
        ).addItems(IEItems.LAVA_FACTORY).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "METALLURGIC_INFUSER_R"),
            1008619, "冶金灌注机", 16
        ).addItems(IEItems.METALLURGIC_INFUSER).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "ANDURIL_R"),
            1008620, "安都瑞尔之剑", 17
        ).addItems(IEItems.ANDURIL).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "ELECTRIC_CROSSBOW_R"),
            1008621, "电动弩", 42
        ).addItems(IEItems.ELECTRIC_CROSSBOW).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "LIGHTNING_BOW_R"),
            1008622, "闪电弓", 37
        ).addItems(IEItems.LIGHTNING_BOW).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "XIONG_SHI_LAO_FANG_R"),
            1008623, "就没有我治不好的玩家", 13
        ).addItems(IEItems.XIONG_SHI_LAO_FANG).register()
        Research(
            NamespacedKey(SlimefunItemExpansion.plugin, "OTHER_MATERIAL_R"),
            1008624, "杂项材料", 12
